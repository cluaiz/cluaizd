...
pub fn run_gc_sweep(env: &LmdbEnv) -> anyhow::Result<()> {
...

    {
        ...
        for result in iter {
            ...
            if let Ok(mut neuron) = serde_json::from_slice::<UniversalNeuron>(val) {
    ...
    if !targets_to_update.is_empty() {
        ...
        for (neuron, compress_lvl) in targets_to_update {
            if neuron.tier == StorageTier::Cold {
                let bytes = serde_json::to_vec(&neuron)?;
                let compressed = encode_all(&bytes[..], compress_lvl as i32)?;
                env.db.put(&mut wtxn, neuron.id.as_bytes().as_slice(), &compressed)?;
            } else {
                let bytes = serde_json::to_vec(&neuron)?;
                env.db.put(&mut wtxn, neuron.id.as_bytes().as_slice(), &bytes)?;
            }
        }

        for id in targets_to_delete {
            env.db.delete(&mut wtxn, id.as_bytes().as_slice())?;
        }
