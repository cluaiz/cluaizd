 ...
 *       if (!db) { fprintf(stderr, "Failed to open CNSDB\n"); return 1; }
 *
 *       // Write a neuron
 *       const char* payload = "{\"name\":\"Aryan\",\"age\":25}";
 *       char* id = cnsdb_write(db, (const uint8_t*)payload, strlen(payload), "text");
 *       if (id) {
 *           printf("Written neuron: %s\n", id);
 *           cnsdb_free_string(id);
 *       }
 *
 *       // Query neurons
 *       char* results = cnsdb_query(db, "Aryan");
 *       if (results) {
 *           printf("Results: %s\n", results);
 *           cnsdb_free_string(results);
 *       }
 *
 *       // Close
 *       cnsdb_close(db);
...
/**
 * Opaque handle to an open CNSDB database instance.
 * Never access the internal fields directly.
 */
typedef struct CnsdbHandle CnsdbHandle;

/**
 * Open a CNSDB database at the given filesystem path.
 *
 ...
 *         Must be freed with cnsdb_close().
 */
CnsdbHandle* cnsdb_open(const char* path, unsigned long map_size_mb);

/**
 * Write a raw payload into CNSDB and get back a unique Neuron ID.
 *
 * @param handle        A valid handle from cnsdb_open().
 ...
 *         MUST be freed with cnsdb_free_string(). Returns NULL on failure.
 */
char* cnsdb_write(
    CnsdbHandle* handle,
...
/**
 * Read a neuron's raw payload from CNSDB by its UUID.
 *
 * @param handle        A valid handle from cnsdb_open().
 ...
 *         MUST be freed with cnsdb_free_bytes(out_len). Returns NULL if not found.
 */
uint8_t* cnsdb_read(
    CnsdbHandle* handle,
...
/**
 * Query CNSDB using a CNQL string or keyword.
...
/**
 * Close a CNSDB handle and release all associated memory.
 *
 * @param handle    A valid handle from cnsdb_open(). NULL-safe.
 */
void cnsdb_close(CnsdbHandle* handle);

/**
 * Free a string returned by cnsdb_write() or cnsdb_query().
 *
 * @param ptr   Pointer returned by cnsdb_write()/cnsdb_query(). NULL-safe.
 */
 ...
 *
 * @param ptr   Pointer returned by cnsdb_read().
 * @param len   The length value written to out_len by cnsdb_read().
 */
void cnsdb_free_bytes(uint8_t* ptr, unsigned long len);
