...
CNSDB was designed from the ground up to be the **universal long-term memory** for autonomous AI agents. It covers all four memory types that modern agents need simultaneously.

---

## The 4 Types of AI Memory — All in One Place

| Memory Type | What AI Needs | How CNSDB Handles It |
|---|---|---|
...
A single CNSDB instance handles all four simultaneously. Your LangChain or AutoGPT agent makes one connection, not four.
...
With CNSDB and the `document_nosql` genome, the agent simply dumps its entire JSON payload directly:
...
This is the most powerful feature of CNSDB for AI. Because Genomes are Rhai scripts embedded in JSON, an AI agent can **write, upload, and activate its own database logic at runtime**.
...
CNSDB solves this with **CNQL Hybrid Pipelines**: hard-filter by structured metadata first, then apply vector similarity only within the matching set.

```text
// WRONG approach (pure vector — may return pink trucks):
similar_to(vector: [car_embedding], top_k: 10)

// RIGHT approach (CNSDB hybrid — guaranteed red cars only):
...
CNSDB exposes a standard HTTP REST API and a 0ms C-FFI. Both are compatible with Python-based agentic frameworks.
...
| Feature | Pinecone / ChromaDB | CNSDB |
|---|---|---|
