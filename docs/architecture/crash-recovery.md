...
CNSDB guarantees durability through its **Write-Ahead Log (WAL)**.

## How the WAL Works
Before any mutation (Insert, Update, Delete) is flushed to the core LMDB storage, it is synchronously appended to the `cnsdb_wal.log` file. 
...
### The Boot Sequence
When the `cnsdb-server` starts, it does not immediately accept HTTP traffic. It undergoes an **Autonomic Boot Sequence**:

1. **Lock Acquisition:** It locks the LMDB environment.
2. **WAL Scan:** It reads `cnsdb_wal.log` sequentially from the beginning.
...
> [!WARNING]
> Do NOT manually delete `cnsdb_wal.log` if the server experienced a hard crash. Doing so will result in permanent data loss for any transactions that were acknowledged to the user but not yet flushed from the OS page cache to the LMDB `.mdb` data files.

## Performance Impact
By default, CNSDB uses asynchronous OS flushes for its WAL to maintain extreme write speeds. If you are operating in a mission-critical financial environment where even 100ms of data loss is unacceptable on power failure, you can configure the WAL to force an `fsync` after every transaction via the `config.toml`.
