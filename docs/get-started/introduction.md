...
Welcome to the official documentation for **CNSDB (Central Nervous System Database)**.
...
**CNSDB shatters this paradigm.**

Built in Rust on top of LMDB, CNSDB is a biological-inspired, autonomic memory substrate. Out of the box, it is a completely empty "Brain" (a graph of Neurons and Synapses). It possesses no hardcoded schemas, no specialized indexes, and no arbitrary limits.
...
Instead of hardcoding features into the core engine, CNSDB relies on **Genomes** (WASM or Rhai scripts). When you insert a Neuron (data record) into CNSDB, you attach a specific Genome to it. 
...
To command these diverse Genomes, CNSDB introduces **CNQL** — a universal, pipeline-based query language. CNQL allows you to run Graph traversals, Vector searches, and Geo-Spatial queries seamlessly within the exact same pipeline string.
